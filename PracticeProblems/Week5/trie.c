...
bool check(char *word)
{
    // Create pointer to traverse trie
    node *trav = root;
    int index;

    for (int i = 0, n = strlen(word); i < n; i++)
    {
        // Index every letter from 0 - 25
        index = tolower(word[i]) - 'a';

        // If pointer at current index is NULL => Name isn't in the dictionary
    ...
    // If the traverse pointer made it's way through the string
    // Check if the name exists in the dictionary and isn't just
